...
`).map(e=>e.split(",").map(Number)).map(([e,t,n])=>({x:e,y:t,z:n})),p2=({x:e,y:t,z:n})=>`${e},${t},${n}`,x3=e=>{const[t,n,r]=e.split(",").map(Number);return{x:t,y:n,z:r}},u6=(e,t)=>Math.sqrt(Math.pow(e.x-t.x,2)+Math.pow(e.y-t.y,2)+Math.pow(e.z-t.z,2)),h4=e=>new Map(e.map(t=>[p2(t),new Set])),p4=e=>e.flatMap(t=>e.filter(n=>p2(t).localeCompare(p2(n))<0).map(n=>({from:p2(t),to:p2(n),dist:u6(t,n)}))).sort(V2(t=>t.dist));function R6(){const e=g4(),t=h4(e);p4(e).slice(0,1e3).forEach(({from:i,to:a})=>{t.get(i).add(a),t.get(a).add(i)});const r=[];for(;t.size;){const i=t.keys().next().value,a=t4(i,o=>[...t.get(o)]);a.values().forEach(o=>t.delete(o)),r.push(a)}const s=r.map(i=>i.size).sort(V2(e4(i=>i))).slice(0,3).reduce(l3);return(()=>{var i=l6(),a=i.firstChild,o=a.nextSibling;return o.nextSibling,j(i,m(T,{children:s}),o),i})()}function c6(){const[e,t]=I(null);return c2(()=>{t(null);const r=function*(){const i=g4(),a=h4(i),o=p4(i),l=new Set(a.keys());let u=0;for(;;){u++,u%500===0&&(yield);const{from:R,to:c}=o.shift();if(a.get(R).add(c),a.get(c).add(R),l.delete(R),l.delete(c),l.size===0&&t4(R,v=>[...a.get(v)]).size===i.length){t(x3(R).x*x3(c).x);return}}}(),s=setInterval(()=>r.next().done&&clearInterval(s),0);n2(()=>clearInterval(s))}),(()=>{var n=L6();return j(n,m(G,{get when(){return e()},fallback:"Loading...",get children(){return["After connecting all points, the product of the x-coordinates of the last connection is ",m(T,{get children(){return e()}}),"."]}})),n})()}const f6=Object.freeze(Object.defineProperty({__proto__:null,Part1:R6,Part2:c6},Symbol.toStringTag,{value:"Module"})),d6=`
98241,50245
98241,51464
98122,51464
98122,52660
